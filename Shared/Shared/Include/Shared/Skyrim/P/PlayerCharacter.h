...
namespace Skyrim
{
	...
	public:
		...
		virtual ObjectReferenceHandle             DropBoundObject(TESBoundObject* boundObject, ExtraDataList* extraDataList, std::uint32_t boundObjectCount, const NiPoint3* position, const NiPoint3* rotation) override; // CB
		...
		BSTArray<ObjectReferenceHandle>                              currentMapMarkerHandles;           // 4F8, 500
		std::uint64_t                                                unknown518;                        // 510, 518
		std::uint64_t                                                unknown520;                        // 518, 520
		std::uint64_t                                                unknown528;                        // 520, 528
		BSTArray<ProjectileHandle>                                   runeHandles;                       // 528, 530
		...
		ActorHandle                                                  actorDoingPlayerCommandHandle;     // 894, 89C
		BSTSmallArray<hkReferencePointer<bhkMouseSpringAction>, 4>   grabSpringActions;                 // 898, 8A0
		ObjectReferenceHandle                                        grabbedReferenceHandle;            // 8C8, 8D0
		...
		ActorHandle                                                  automaticAimActorHandle;           // 9B8, 9C0
	...
	static_assert(offsetof(PlayerCharacter, travelCommandLocationMarkerHandle) == SKYRIM_RELOCATE(0x440, 0x448));
	...
	static_assert(offsetof(PlayerCharacter, currentMapMarkerHandles) == SKYRIM_RELOCATE(0x4F8, 0x500));
	static_assert(offsetof(PlayerCharacter, runeHandles) == SKYRIM_RELOCATE(0x528, 0x530));
	...
	static_assert(offsetof(PlayerCharacter, actorDoingPlayerCommandHandle) == SKYRIM_RELOCATE(0x894, 0x89C));
	static_assert(offsetof(PlayerCharacter, grabSpringActions) == SKYRIM_RELOCATE(0x898, 0x8A0));
	static_assert(offsetof(PlayerCharacter, grabbedReferenceHandle) == SKYRIM_RELOCATE(0x8C8, 0x8D0));
